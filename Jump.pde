...
float timefix=3;
//time fix should be 1 for running in browser
int level=0;
float nX=0;
float nY=0;
color c1=#00bfff;
...
float timeconvert(float x)
  {
  return(x*timefix*frameRate);    
  }
class runner
{
  color Color;
...
powerup()
    ...
    void update()
      {
        //if ((second()-nowtime)%15==0)
        //activates powerup after some random time
          {
          //active=true;
          }
